...
def grow(island, coordinates, chance):
    # if at edge of map
    if coordinates['y'] == 0 or coordinates['y'] == len(island) - 1 or coordinates['x'] == 0 or coordinates['x'] == len(
            island[0]) - 1:
        ...
        if island[coordinates['y'] - 1][coordinates['x']] == 0:
            if rand(100) < chance['north'] or coordinates['y'] - 1 == 0:
                island[coordinates['y'] - 1][coordinates['x']] = 2
            else:
                grow(island, {'x': coordinates['x'], 'y': coordinates['y'] - 1}, chance)
        # east
        if island[coordinates['y']][coordinates['x'] + 1] == 0:
            if rand(100) < chance['east'] or coordinates['x'] + 1 == len(island[0]):
                island[coordinates['y']][coordinates['x'] + 1] = 2
            else:
                grow(island, {'x': coordinates['x'] + 1, 'y': coordinates['y']}, chance)
        # south
        if island[coordinates['y'] + 1][coordinates['x']] == 0:
            if rand(100) < chance['north'] or coordinates['y'] + 1 == 0:
                island[coordinates['y'] + 1][coordinates['x']] = 2
...
def shore_tile(island, tileset, row, col):
    ...
    if row == 0 or row != 0 and island[row - 1][col] == 0:
        if col == 0 or col != 0 and island[row][col - 1] == 0:
            return get_tile(tileset['corner'])
        elif col == island_width or col != island_width and island[row][col + 1] == 0:
            return rotate(get_tile(tileset['corner']), 90)
        else:
            return get_tile(tileset['side'])
    if row == island_height or row != island_height and island[row + 1][col] == 0:
        if col == 0 or col != 0 and island[row][col - 1] == 0:
            return rotate(get_tile(tileset['corner']), 270)
        elif col == island_width or col != island_width and island[row][col + 1] == 0:
            return rotate(get_tile(tileset['corner']), 180)
        else:
            return rotate(get_tile(tileset['side']), 180)
    if col == 0 or col != 0 and island[row][col - 1] == 0: return rotate(get_tile(tileset['side']), 270)
    if col == island_width or col != island_width and island[row][col + 1] == 0:
        return rotate(get_tile(tileset['side']), 90)
    else:
        return rotate(get_tile(tileset['inland']), rand(4) * 90)
...
def tiles(island, tileset):
    for row in range(len(island)):
        for col in range(len(island[0])):
            if island[row][col] == 1:
                island[row][col] = rotate(get_tile(tileset['mainland']), rand(4) * 90)
            elif island[row][col] == 2:
                island[row][col] = shore_tile(island, tileset, row, col)
